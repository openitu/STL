     ...
     char *type;
{
  ...
  /* Final check to see if the input bitstream is a byte-oriented G.192 bitstream. In this case, the first byte is 0x2n (n=0..F) and the second byte must be the frame length */
     ...
     long *fr_len;
{
  ...
  *bs_format = check_bs_format (F, file, bs_type);
  ...
  /* If the BS seems to have a header, search for two consecutive ones. If it finds, determines which is the frame size */
  ...
  if (sync_flag == sync_header) {
    ...
    /* Use the one found; use as no.of bytes read if BYTE of no.of words if G192 */
     ...
     char *argv[];
{
  ...
  /* Find the actual frame size for the serial bit stream from the serial bitstream. If this is a headerless bitstream, assumes that the frame size is based on the provided block size. */

  /* Find the number of words per frame (payload only!), check whether a sync header was found, and also returns the type of bitstream (g192, byte, compact) found */
  ...
  /* Use the proper data I/O functions; format of output file is the same as that of the input file */
  read_data = bs_format == byte ? read_byte : (bs_format == g192 ? read_g192 : read_bit_ber);
  save_data = bs_format == byte ? save_byte : (bs_format == g192 ? save_g192 : save_bit);
  ...
  /* Move file's pointer to 1st desired block (since the BS probing has changed the FILE pointer) */
  if (fseek (Fibs, start_byte, 0) < 0l)
    KILL (inpfil, 4);
