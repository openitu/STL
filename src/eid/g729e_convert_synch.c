  ...
  while (fread (serial, sizeof (short), 2, f_serial) == 2) {
    ...
    if (serial_size == 0) {
      if (serial[0] == G192_SYNC) {
        ...
        if (!quiet) {
          printf ("\nDTX::zero length payload, synch word(%x) in file %s , frame %ld ", serial[0], argv[1], frame);
        }
      } else {                  /* not synch */
    ...
    if (serial[0] != G192_SYNC) {
      nb_fer_in++;
      if (!quiet) {
        printf ("\nFER::non synch word(%x) in file %s , frame %ld ", serial[0], argv[1], frame);
      }
      fer = 1;
    }
    ...
    if ((serial[0] == G192_SYNC) && (serial[1] != 0)) {
      ...
      if (zeroed) {
        nb_fer_zeroed_in++;
        if (!quiet) {
          printf ("\nFER::zeroed payload, synch word(%x) in file %s , frame %ld ", serial[0], argv[1], frame);
        }
        fer = 1;
      }
    }
