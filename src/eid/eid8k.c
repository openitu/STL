     ...
     char *argv[];
{
  ...
  double BER_gamma = 0.0, FER_gamma = 0.0;      /* burst factors. NOT NEEDED */
  ...
  GET_PAR_C (2, "_Processing mode (Random,Frame,Burst) [RFB] .......: ", mode);
  GET_PAR_L (3, "_Number of frames .................................: ", number_of_frames);
  FIND_PAR_L (4, "_Start frame ......................................: ", start_frame, 1);
  ...
  case 'f':
    {
      ...
      for (i = start_frame; i <= number_of_frames; i++) {
        for (j = 0; j <= 1; j++) {
          ...
          genfrms += 1.0;
          if (ber1 == 0.0) {
            memcpy (&ybuff[j * EID_BUFFER_LENGTH], &frame_okay[0], EID_BUFFER_LENGTH);
          } else {
            memcpy (&ybuff[j * EID_BUFFER_LENGTH], &frame_erased[0], EID_BUFFER_LENGTH);
          }
        }
        memcpy (&ybuff[2 * EID_BUFFER_LENGTH], &zero_vector[0], ZERO_VECTOR_LENGTH);
        items = fwrite (&ybuff[0], sizeof (char), 512, out_file_ptr);
