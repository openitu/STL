...
int main (int argc, char *argv[]) {
  ...
  while (argv[1][0] == '-') {   /* yes, it's an option! */
    if (!strcmp (argv[1], "-ILS") || !strcmp (argv[1], "-ils")) {       /* valid option: ILS header */
      isILS = 1;
      argv++;
      argc--;
  ...
  if (overwrite) {
    if (conditional_swap == endian_type () || conditional_swap == ANY)
      ...
      fprintf (stderr, "%s: Skipping byte swapping, system is %s\n", argv[1], endian_type () == BIG_ENDIAN ? "big endian" : "little endian");
      exit (0);
    }
  }
  ...
  if (s.st_size != total)
    fprintf (stderr, "Problem: input file size is %ld, converted bytes are %ld\n", s.st_size, total);
...
#ifdef OLD_WAY
#ifdef MSDOS
    if (chmod (argv[1], (int) 0700) || unlink (argv[1]) || rename (fileout, argv[1]))
      KILL (argv[1], 10);
#else
    char cmd[200];
...
#endif
#else
    if (chmod (argv[1], (int) 0700) || remove (argv[1]) || rename (fileout, argv[1]))
      KILL (argv[1], 10);
#endif
  }
#ifndef VMS
