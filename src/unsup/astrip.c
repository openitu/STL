    ...
    if (smooth) {
      if (tot_samples < w_len) {
        ...
        for (n = 0; n < N && start_count < w_len; n++, start_count++) {
          buf[n] = (short) ((float) buf[n] * (w_alpha - w_alpha_1 * cos ((start_count) * factor)));
        }
#ifdef DEBUG
      ...
      /* updated only in the end of the loop */
        ...
        for (n = 0, abs_n = up_lim > tot_samples ? up_lim : tot_samples; n + 1 < N; abs_n++, end_count++) {
          n = abs_n % N;
          buf[n] = (short) ((float) buf[n] * (w_alpha - w_alpha_1 * cos ((w_len + end_count) * factor)));
        }
#else
        ...
        /* whole block til end of segment */
        n = 0;
        abs_n = up_lim > tot_samples ? up_lim : tot_samples;
        do {
          n = abs_n % N;
          buf[n] = (short) ((float) buf[n] * (w_alpha - w_alpha_1 * cos ((w_len + end_count + 1) * factor)));
          abs_n++;
          end_count++;
        } while (n + 1 < N);
#endif /* ORI_WAY */
