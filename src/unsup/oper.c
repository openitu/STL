...
int main (int argc, char *argv[]) {
  ...
  if (argc < 2)
    ...
    while (argc > 1 && argv[1][0] == '-')
      if (strcmp (argv[1], "-delay") == 0) {
        ...
        if (strcmp (argv[2], "dB") == 0 || strcmp (argv[2], "DB") == 0 || strcmp (argv[2], "db") == 0) {
          gain_in_dB = 1;
        ...
        /* Order to trim output file size by the size of file 1, 2, or a compromise of both [default] */
        trim_by = atoi (argv[2]);
  ...
  FIND_PAR_C (11, "Short(I), long(L), float (R) or double (D) data? .. ", c[0], TypeOfData);
  TypeOfData = c[0];
