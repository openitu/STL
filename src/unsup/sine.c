...
int main (int argc, char *argv[]) {
  ...
  if (argc < 2)
    ...
    while (argc > 1 && argv[1][0] == '-')
      if (strcmp (argv[1], "-n") == 0) {
        ...
        argc -= 2;
        argv += 2;
      } else if (strcmp (argv[1], "-dB") == 0 || strcmp (argv[1], "-db") == 0 || strcmp (argv[1], "-DB") == 0) {
        /* AC level is in dB */
        use_dB = 1;
  ...
  if (AClevel == -100000) {
    ...
    argc++;
    argv++;
    fprintf (stderr, "%s%-8.2f\n", use_dB ? "_AC level? [dBov] ..................... " : "_Tone peak value? [PCM] ............... ", AClevel);
  }
  if (f < 0) {
  ...
  FIND_PAR_I (9, "_A/D resolution [bits] ................ ", AD_resolution, AD_resolution);
  ...
  if (use_dB) {
    AClevel = OverFlow * sqrt (2.0) * pow ((double) 10.0, (double) (AClevel) / (double) 20.0);
  }
#ifdef DEBUG
  printf ("ACL=%f\n", AClevel);
#endif
