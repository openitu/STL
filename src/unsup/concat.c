...
int main (int argc, char *argv[]) {
  ...
  while (argc > 2) {
    ...
    if (undo) {
      ...
      /* If the filesize is odd, then the file is organized in byte-boundaries, not 16- or 32-bit words, and the concatenated file cannot be separated using the default STL command, astrip. Hence, prints a warning to the user. */
      if (st.st_size % 2) {
        fprintf (Fundo, "echo WARNING! File %s has an odd no.of samples. \n", cur_file);
        fprintf (Fundo, "echo %s%s", "The commands below are offset (late) ", "by (at least) one byte.\n");
        fprintf (Fundo, "echo %s%s %s!\n", "From this point on, you cannot ", "use astrip to separate", file_out);
        bad_undo++;
      }
