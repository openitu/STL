...
int main (int argc, char *argv[]) {
  ...
  int N, N1, N2;
  char FileIn[50], KindOfDump = { 'D' }, TypeOfData = {
  'I'};
  short int ilet[4096];
  char quiet;
#ifdef VMS
  ...
  FIND_PAR_C (5, " Decimal or Hex dump? (D,H) [D] .................... ", C[0], KindOfDump);
  KindOfDump = (toupper (C[0]) != 'H') ? 'D' : 'H';
  FIND_PAR_C (6, " Short(I), long(L), float (R) or double (D) data? .. ", C[0], TypeOfData);
  TypeOfData = C[0];
