  ...
  while (argc > 1) {
    ...
    if (!quiet) {
      ...
      if (SVP56_get_activity (state) == 0) {
        ...
        fprintf (stderr, "%s%s", "\n ---------------------------", "----------------------------");
        fprintf (stderr, "\n  DC level: ..................... %7.0f [PCM]", Overflow * SVP56_get_DC_level (state));
        fprintf (stderr, "\n  Maximum positive value: ....... %7.0f [PCM]", Overflow * SVP56_get_pos_max (state));
        fprintf (stderr, "\n  Maximum negative value: ....... %7.0f [PCM]", Overflow * SVP56_get_neg_max (state));
        fprintf (stderr, "%s%s", "\n ---------------------------", "----------------------------");
        fprintf (stderr, "\n  Noise/silence energy (rms): ... %7.3f [dB]", SVP56_get_rms_dB (state));
      } else {
        fprintf (stderr, "%s%s", "\n ---------------------------", "----------------------------");
        ...
        fprintf (stderr, "%s%s", "\n ---------------------------", "----------------------------");
        fprintf (stderr, "\n  Long term energy (rms): ....... %7.3f [%s]", SVP56_get_rms_dB (state), unity);
        fprintf (stderr, "\n  Active speech level: .......... %7.3f [%s]", ActiveLeveldB, unity);
        fprintf (stderr, "\n  RMS peak-factor found: ........ %7.3f [dB]", abs_max_dB - SVP56_get_rms_dB (state));
        fprintf (stderr, "\n  Active peak factor found: ..... %7.3f [dB]", abs_max_dB - ActiveLeveldB);
        fprintf (stderr, "\n  Activity factor: .............. %7.3f [%%]", SVP56_get_activity (state));
      }
      fprintf (stderr, "%s%s", "\n ---------------------------", "----------------------------\n");
    } else {
      ...
      /* Skip if filesize is zero */
      if (state.n == 0) {
        printf ("%s%s", "Min: ------ Max: ----- DC: ------- ", "RMSLev[dB]: ------- ActLev[dB]: ------- %Active:  ------");
      } else {
    ...
    /* ... PRINT-OUT OF RESULTS ... */
    if (!quiet)
      print_act_long_summary (out, FileIn, state, ActiveLeveldB, level, Overflow, gain, N, N1, N2, bitno);
    else
