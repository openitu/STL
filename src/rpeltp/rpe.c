 ...
 */
{
  ...
  /* for (k = 0; k <= 4; k++) wt[k] = 0; for (k = 5; k <= 44; k++) wt[k] = *e++; for (k = 45; k <= 49; k++) wt[k] = 0; (e[-5..-1] and e[40..44] are allocated by the caller, are initially zero and are not written anywhere.) */
  e -= 5;
  ...
  for (k = 0; k <= 39; k++) {
    ...
    /* for (i = 0; i <= 10; i++) { L_temp = GSM_L_MULT( wt[k+i], gsm_H[i] ); L_result = GSM_L_ADD( L_result, L_temp ); } */
...
#define	STEP( i, H )	(e[ k + i ] * (longword)H)
    ...
    /* Every one of these multiplications is done twice -- but I don't see an elegant way to optimize this.  Do you? */
    ...
    /* L_result = GSM_L_ADD( L_result, L_result ); (* scaling(x2) *) L_result = GSM_L_ADD( L_result, L_result ); (* scaling(x4) *) x[k] = SASR( L_result, 16 ); */

    /* 2 adds vs. >>16 => 14, minus one shift to compensate for those we lost when replacing L_MULT by '*'. */
 ...
 */
{
  ...
  /* for (m = 0; m <= 3; m++) { L_result = 0; for (i = 0; i <= 12; i++) { temp1 = SASR( x[m + 3*i], 2 ); assert(temp1 != MIN_WORD); L_temp = GSM_L_MULT( temp1, temp1 ); L_result = GSM_L_ADD( L_temp, L_result ); } if (L_result > EM) { Mc = m; EM = L_result; } } */
...
static void APCM_quantization P5 ((xM, xMc, mant_out, exp_out, xmaxc_out), word * xM,   /* [0..12] IN */
  ...
  /* This computation uses the fact that the decoded version of xmaxc can be calculated by using the exponent and the mantissa part of xmaxc (logarithmic table).  So, this method avoids any division and uses only a scaling of the RPE samples by a function of the exponent.  A direct multiplication by the inverse of the mantissa (NRFAC[0..7] found in table 4.5) gives the 3 bit coded version xMc[0..12] of the RPE samples. */
...
static void APCM_inverse_quantization P4 ((xMc, mant, exp, xMp), register word * xMc,   /* [0..12] IN */
                                          word mant, word exp, register word * xMp) {   /* [0..12] OUT */
  /* 
