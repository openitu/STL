...
void fprint_word P2 ((f, w), FILE * f, word w) {
  if (!w)
    ...
    fprintf (f, "0x%4.4x (%d%s)", (unsigned int) w, (int) w, w == MIN_WORD ? "/-" : (w == MAX_WORD ? "/+" : ""));
}
...
void fprint_longword P2 ((f, w), FILE * f, longword w) {
  if (!w)
    ...
    fprintf (f, "0x%8.8x (%ld%s)", w, w, w == MIN_WORD ? "/-" : (w == MAX_WORD ? "/+" : (w == (longword) MIN_LONGWORD ? "/--" : (w == (longword) MAX_LONGWORD ? "/++" : ""))));
}
...
void do_longword P1 ((w), longword w) {
  ...
  if (do_expect) {
    if (w != L_expect) {
      ...
      fprintf (stderr, " != %s (%ld, %ld) -- expected ", opname, L_op1, L_op2);
