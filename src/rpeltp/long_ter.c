 ...
 */
 ...
 /* This procedure exists in four versions.  First, the two integer versions with or without table-multiplication (as one function); then, the two floating point versions (as another function), with or without scaling. */
...
static void Calculation_of_the_LTP_parameters P4 ((d, dp, bc_out, Nc_out), register word * d,   /* [0..39] IN */
  ...
  for (lambda = 40; lambda <= 120; lambda += 9) {
    ...
    register float W;
    register float a = lp[-8], b = lp[-7], c = lp[-6], d = lp[-5], e = lp[-4], f = lp[-3], g = lp[-2], h = lp[-1];
    register float E;
...
#		define	STEP_H(K)	STEP(K, h, a, b, c, d, e, f, g)
  ...
  *Nc_out = Nc;
  ...
  /* Coding of the LTP gain Table 4.3a must be used to obtain the level DLB[i] for the quantization of the LTP gain b to get the coded version bc. */
