...
static void Decoding_of_the_coded_Log_Area_Ratios P2 ((LARc, LARpp), word * LARc,       /* coded log area ratio [0..7] IN */
  ...
  /* This procedure requires for efficient implementation two tables.  INVA[1..8] = integer( (32768 * 8) / real_A[1..8]) MIC[1..8] = minimum value of the LARc[1..8] */

  /* Compute the LARpp[1..8] */

  /* for (i = 1; i <= 8; i++, B++, MIC++, INVA++, LARc++, LARpp++) { temp1 = GSM_ADD( *LARc, *MIC ) << 10; temp2 = *B << 1; temp1 = GSM_SUB( temp1, temp2 ); assert(*INVA != MIN_WORD); temp1 = GSM_MULT_R( *INVA, temp1 ); *LARpp = GSM_ADD( temp1, temp1 ); } */
...
static void Coefficients_13_26 P3 ((LARpp_j_1, LARpp_j, LARp), register word * LARpp_j_1, register word * LARpp_j, register word * LARp) {
  register int i;
  register longword ltmp;
...
static void LARp_to_rp P1 ((LARp), register word * LARp) {      /* [0..7] IN/OUT */
  ...
  for (i = 1; i <= 8; i++, LARp++) {
    ...
    /* temp = GSM_ABS( *LARp ); if (temp < 11059) temp <<= 1; else if (temp < 20070) temp += 11059; else temp = GSM_ADD( temp >> 2, 26112 ); *LARp = *LARp < 0 ? -temp : temp; */
...
static void Short_term_synthesis_filtering P5 ((S, rrp, k, wt, sr), struct gsm_state *S, register word * rrp,   /* [0..7] IN */
  ...
  while (k--) {
    ...
    for (i = 8; i--;) {
      ...
      tmp1 = rrp[i];
      tmp2 = v[i];
      tmp2 = (tmp1 == MIN_WORD && tmp2 == MIN_WORD ? MAX_WORD : 0x0FFFF & (((longword) tmp1 * (longword) tmp2 + 16384) >> 15));

      sri = GSM_SUB (sri, tmp2);

      /* v[i+1] = GSM_ADD( v[i], gsm_mult_r( rrp[i], sri ) ); */
      tmp1 = (tmp1 == MIN_WORD && sri == MIN_WORD ? MAX_WORD : 0x0FFFF & (((longword) tmp1 * (longword) sri + 16384) >> 15));
