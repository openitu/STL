...
int main (int argc, char *argv[]) {
  ...
  if (argc < 2)
    ...
    while (argc > 1 && argv[1][0] == '-')
      if (strcmp (argv[1], "-start") == 0) {
        ...
        argv++;
        argc--;
      } else if (strcmp (argv[1], "-right") == 0 || strcmp (argv[1], "-RIGHT") == 0) {
  ...
  if (inp_type == IS_SERIAL && out_type == IS_PARALLEL) {       /* SP mode: */
    ...
    /* Output frame size is the # of bits per frame divided by the resolution */
    Nout = Nin / resolution;
  ...
  } else {                      /* PS mode: Input is parallel and output is serial */
  ...
  if (inp_type == IS_SERIAL && out_type == IS_PARALLEL) {
    ...
    for (cur_blk = 0; cur_blk < N2; cur_blk++) {
      ...
      i += parallelize_f (bit_stm,      /* input buffer pointer */
    ...
    for (cur_blk = 0; cur_blk < N2; cur_blk++) {
      ...
      if ((smpno = fread (par_buf, sizeof (short), Nin, inpfilptr)) < 0)
        KILL (inpfil, 5);
      bitno += serialize_f (par_buf,    /* input buffer pointer */
