...
int main (int argc, char **argv) {
  ...
  if ((mode != 'e' && mode != 'd' && mode != 'c') || (l != 'a' && l != 'u' && l != 'A' && l != 'U')) {
    error (badParamMsg);
  }

  if ((in = fopen (argv[3 + sh], "rb")) == NULL || (out = fopen (argv[4 + sh], "wb")) == NULL) {
  ...
  if (argc < 2)
    ...
    while (argc > 1 && argv[1][0] == '-')
      if (strcmp (argv[1], "-frame") == 0 || strcmp (argv[1], "-blk") == 0) {
        /* Get skip length */
        N = atol (argv[2]);
        ...
        argv += 2;
        argc -= 2;
      } else if (strcmp (argv[1], "-frame") == 0 || strcmp (argv[1], "-blk") == 0) {
  ...
  fprintf (stderr, "Operation: %s; Nc=%d; Ne=%d\n", encode && decode ? "Enc+Dec" : (encode ? "Encode-only" : "Decode-only"), nc, ne);
 ...
 */
  ...
  for (cur_blk = 0; cur_blk < N2; cur_blk++) {
    ...
    if (encode && !decode)
      g727_encode (inp_buf, out_buf, smpno, law, nc, ne, &enc_state);
    else if (decode && !encode)
      g727_decode (inp_buf, out_buf, smpno, law, nc, ne, &dec_state);
    else if (encode && decode) {
      g727_encode (inp_buf, tmp_buf, smpno, law, nc, ne, &enc_state);
      g727_decode (tmp_buf, out_buf, smpno, law, nc, ne, &dec_state);
    }
