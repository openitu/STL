 ...
 */
void g728_cbsearch16 (Float * t,        /* gain-normalized target */
 ...
 */
int g728_hybridw (int lpc,      /* linear predictor order */
  ...
  for (i = 0; i <= lpc; ++i) {  /* autocorrelations */
    /* recursive part */
    rexp[i] = atten * rexp[i] + g728_vdotp (&ws[nonr], &ws[nonr + i], nfrsz);
    /* non-recursive part */
...
void g728_impres (Float * a,    /* LPC filter coeffs */
 ...
 */
void g728_lgpred (Float * gp,   /* log-gain predictor coefficients */
 ...
 */
void g728_normsq (Float y[][IDIM],      /* excitation VQ shape codebook */
 ...
 */
void g728_synmem (Float * a,    /* LPC filter coeffs */
 ...
 */
void g728_synmemrx (Float * a,  /* LPC filter coeffs */
                    Float * statelpc,   /* LPC filter memory */
...
void g728_wfilter (Float * b, Float * a, Float * bmem, Float * amem) {
  ...
  for (i = 0; i < IDIM; i++, bmem--) {
    s = bmem[0] + g728_vdotp (b, &bmem[1], LPCW) - g728_vdotp (a, amem, LPCW);
...
void g728_zirlpcf (Float * a,   /* LPC filter coeffs */
                   Float * statelpc) {  /* LPC filter memory */
  int i;
  Float f;
