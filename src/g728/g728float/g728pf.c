...
void g728_ltpfcoef (int kp,     /* detected pitch period */
                    Float * gl, /* long-term postfilter gain scaling factor */
                    Float * glb,        /* gl*b, where b is the long-term pf coeff. */
                    Float * rtap,       /* optimal single-tap pitch predictor */
                    Float * pfmem, Float pzcf) {
  Float tmp, sum;
...
void g728_decp (int *kp,        /* pitch period of previous(in) and current(out) frame */
                Float * dec, Float * pitchd, Float * lpfiir) {
  int i, j;
