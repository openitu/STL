 ...
 */
void g728decode (Float * speech,        /* output speech */
  ...
  for (; sz >= IDIM; sz -= IDIM) {      /* loop through vectors */
    ...
    if (d->ferased)             /* handle frame erasures */
      ...
      g728_lgpred (d->gp, d->gstate, &gain, &gaindb, d->ofecount, &d->ogaindb);
    ...
    if (d->postf) {
      ...
      if (d->count == UPDATEKP) {       /* update pitch estimate */
        ...
        g728_ltpfcoef (d->kp, &d->gl, &d->glb, &d->tap, d->pfmem, d->pzcf);
        d->resid = &d->pitchd[NFRSZ - IDIM];
      } else
        d->resid -= IDIM;
      g728_postf (d->kp, d->gl, d->glb, d->az, d->ap, d->tiltz, d->statelpc, stpf, d->pfmem, d->stpffir, d->stpfiir, &d->scalefil);
      fp = stpf;
    } else
      fp = d->statelpc;
    for (i = IDIM - 1; i >= 0; i--)
    ...
    if (d->count == UPDATELPC) {        /* calculate LPC coeff */
      d->info = 0;
      if (g728_hybridw (LPC, NFRSZ, NONR, (Float) ATTENL, g728rom.wnr, d->sb, d->rexp, d->r) &&
          /* lpc analysis of coded speech up to order 10 */
          (d->info = g728_durbin (d->r, 0, LPCW, d->atmp, &rc, &d->alpha))) {
        if (d->postf) {
          /* 
          ...
          g728_vmult (d->apf, d->spfzcfv, G728_LPCPF, d->az);
          g728_vmult (d->apf, g728rom.spfpcfv, G728_LPCPF, d->ap);
        }
      }
      ...
      if (d->info && d->fecount <= 1 && (d->info = g728_durbin (d->r, LPCW, LPC, d->atmp, &rc, &d->alpha)))
        g728_vmult (d->atmp, g728rom.facv, LPC, d->atmp);
      if (g728_hybridw (LPCLG, NUPDATE, NONRG, (Float) ATTENG, g728rom.wnrg, d->sbg, d->rexpg, d->r) && !d->ferased && g728_durbin (d->r, 0, LPCLG, gptmp, &rc, &d->alpha))
        /* update gain pred */
 ...
 */
void g728decfe (Float * speech, /* output speech */
                int sz,         /* output size - must be multiple of IDIM */
                G728DecData * d) {      /* RAM */
  int i;
  Short ix;
