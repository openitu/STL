 ...
 */
void g728vocode (Float * speech,        /* output speech */
  ...
  for (; sz >= IDIM; sz -= IDIM) {      /* loop through vectors */
    ...
    *--d->gstate = d->gq_db[ig] + g728rom.y_db[is] + gaindb;
    ...
    if (d->count == UPDATELPC) {        /* calculate LPC coeff */
      d->info = 0;
      if (g728_hybridw (LPC, NFRSZ, NONR, (Float) ATTENL, g728rom.wnr, d->sb, d->rexp, r) &&
      ...
      if (g728_hybridw (LPCW, NFRSZ, NONRW, (Float) ATTENW, g728rom.wnrw, d->sbw, d->rexpw, r) && g728_durbin (r, 0, LPCW, awztmp, &rc, &alpha)) {
      ...
      if (g728_hybridw (LPCLG, NUPDATE, NONRG, (Float) ATTENG, g728rom.wnrg, d->sbg, d->rexpg, r) && g728_durbin (r, 0, LPCLG, gptmp, &rc, &alpha))
        /* update gain pred */
