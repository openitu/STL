 ...
 */
void g728fp_encode (Short * index,      /* output indices */
    ...
    *--d->gstate = (*d->lgupdate) (gaindb, ig, is);
    ...
    if (d->count == UPDATELPC) {        /* calculate LPC coeff */
      ...
      if (g728fp_block49 (d->sb, d->rexp, &d->nlsrexp, d->r))
        ...
        d->info = g728fp_durbin (d->r, 0, LPCPF, d->ja, &rc, &d->alpha, &d->qtmp);
      ...
      if (g728fp_block36 (d->sbw, d->rexpw, &d->nlsrexpw, d->r) && g728fp_durbin (d->r, 0, LPCW, jawztmp, &rc, &d->alpha, &d->qtmp) && g728fp_weight (jawztmp, awptmp, LPCW, iwwp, d->qtmp) && g728fp_weight (jawztmp, awztmp, LPCW, iwwz, d->qtmp)) {
    ...
    /* calculate new gain predictor coeff. */
      ...
      if (d->info && (d->info = g728fp_durbin (d->r, LPCPF, LPC, d->ja, &rc, &d->alpha, &d->qtmp)))
        d->info = g728fp_weight (d->ja, d->atmp, LPC, g728fp_iw, d->qtmp);
      if (g728fp_block43 (d->sbg, d->rexpg, &d->nlsrexpg, d->r) && g728fp_durbin (d->r, 0, LPCLG, d->ja, &rc, &d->alpha, &d->qtmp) && g728fp_weight (d->ja, gptmp, LPCLG, g728fp_iwg, d->qtmp))
