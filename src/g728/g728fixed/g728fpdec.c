 ...
 * decode vectors
 */
void g728fp_decode (Short * speech,     /* output speech */
  ...
  for (; sz >= IDIM; sz -= IDIM) {      /* loop through vectors */
    ...
    if (++d->count == NUPDATE) {
      ...
      if (d->ferased) {
         ...
         */
        if ((d->fecount & 3) == 1)
          g728fp_erasedsetup (d->kp, d->tap, d->fecount >> 2, d);
      }
    }
    if (d->ferased)
      ...
      g728fp_lgpred (d->gp, d->gstate, &gain, &gaindb, d->ofecount, d->ogaindb);
      g728fp_zirlpcf (d->a, d->statelpc);       /* zero LPC input response */
    ...
    if (d->postf) {
      ...
      if (d->count == UPDATEKP) {       /* update the pitch estimate */
        ...
        d->resid -= IDIM;
      g728fp_postf (d->kp, d->gl, d->glb, d->az, d->ap, d->tiltz, st, stpf, d->stpffir, d->stpfiir, &d->scalefil, d->pfmem);
    ...
    if (d->count == UPDATELPC) {        /* calculate LPC coeff */
      ...
      if (g728fp_block49 (d->sb, d->rexp, &d->nlsrexp, d->r) &&
          ...
          (d->info = g728fp_durbin (d->r, 0, LPCPF, d->ja, &rc, &d->alpha, &d->qtmp)) && d->postf) {
    ...
    /* calculate new gain predictor coeff. */
      ...
      if (d->info && d->fecount <= 1 && (d->info = g728fp_durbin (d->r, LPCPF, LPC, d->ja, &rc, &d->alpha, &d->qtmp)))
        d->info = g728fp_weight (d->ja, d->atmp, LPC, g728fp_iw, d->qtmp);
      if (g728fp_block43 (d->sbg, d->rexpg, &d->nlsrexpg, d->r) && !d->ferased && g728fp_durbin (d->r, 0, LPCLG, d->ja, &rc, &d->alpha, &d->qtmp) && g728fp_weight (d->ja, gptmp, LPCLG, g728fp_iwg, d->qtmp))
 ...
 */
void g728fp_eraseframe (Short * speech, /* output speech */
