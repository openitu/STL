...
short g192_to_byte (short nb_bits, short *code_byte, short *inp_bits, short N_samples) {
  ...
  if (nb_bits == (N_samples / 2) * 7) {
    mode = 2;
  };
  if (nb_bits == (N_samples / 2) * 6) {
    mode = 3;
  };
     ...
     char *argv[];
{
  ...
  if (g192_inp) {
    ...
    /* Do preliminary inspection in the INPUT BITSTREAM FILE to check that it has a correct format (g192) */
    ...
    while (fread (header, sizeof (short), 2, F_cod) == 2) {
      if (!((short) fread (inp_frame, sizeof (short), header[1], F_cod) == header[1])) {
        ...
        if (header[0] != G192_SYNC) {   /* bad frame, (with zero length or valid G.722 length) */
          ...
          good_cnt = 0;
          if (debug) {
            fprintf (stderr, "PLC-FER fer_cnt=%ld \n", fer_cnt);
          };
    ...
    /* for non g192 input, simply read input bits in chunks of bytes of N/2 bytes stored in words, always read 64 kbps */
