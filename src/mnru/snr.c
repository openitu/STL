...
typedef struct {
  ...
  double total_err, total_snr_dB, total_sqr_dB, max_sqr, min_sqr, avg_sqr, avg_snr, var_sqr, var_snr, max_snr, min_snr, total_sqr, thr, sqr, err;
 ...
 */
  ...
  if (mode == SNR_DEALLOC) {
    ...
    if (mode == SNR_STOP) {
      ...
      s->total_snr_dB = (s->total_err == 0) ? 100 /* dB */ :
