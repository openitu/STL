...
The software package was reworked to make it available as a truly open-source projects and is therefore hosted on an open-source collaboration platform. The build toolchain is now using CMake to generate platform-dependent and tool-dependent build scripts as well as to execute regression tests for each module in the STL.
