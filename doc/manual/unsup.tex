...
\[
  \text{seed}_{n+1} = (69069 \times \text{seed}_n + 1) \mod 2^{32}
\]

The generator returns a uniformly distributed value in $[0, 1)$ by
scaling:
\[
  r = 2^{-32} \times \text{seed}
\]
...
inclusive range $[\text{start}, \text{stop}]$. Each value is computed
as:
\[
  v = \text{start} + \lfloor (\text{stop} - \text{start} + 1) \cdot r \rfloor
\]
...
\item[\tt -r START STOP]
        ...
        $[\text{START}, \text{STOP}]$. When this option is specified,
